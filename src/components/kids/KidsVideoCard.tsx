  ...
  return (
    <Link href={`/watch/${id}`} className="block">
      <div className={`${isDark ? 'bg-zinc-900/50 hover:bg-zinc-900' : 'bg-white hover:shadow-md'} sm:rounded-3xl overflow-hidden shadow-sm hover:scale-[1.01] transition-all duration-300`}>
        ...
        {/* Info Container */}
        <div className="p-3 sm:p-4 landscape:p-2 flex gap-2 sm:gap-3 items-start">
          {/* Channel/Avatar placeholder */}
          <div className="w-8 h-8 sm:w-10 sm:h-10 landscape:hidden rounded-full bg-gradient-to-tr from-primary to-blue-400 flex-shrink-0 border-2 border-white/20 shadow-sm flex items-center justify-center text-white font-bold text-xs">
            KT
          </div>
          
          <div className="flex-1 min-w-0 pt-0.5">
            <h3 className={`font-bold text-sm sm:text-base landscape:text-xs leading-tight line-clamp-2 mb-1 ${isDark ? 'text-white' : 'text-foreground'}`}>
              {title}
            </h3>
            <p className={`text-[10px] sm:text-xs font-medium flex items-center gap-1.5 ${isDark ? 'text-white/60' : 'text-muted-foreground'}`}>
              <span className="landscape:hidden">Khalifah TV</span>
              <span className={`w-1 h-1 rounded-full landscape:hidden ${isDark ? 'bg-white/30' : 'bg-muted-foreground/40'}`} />
